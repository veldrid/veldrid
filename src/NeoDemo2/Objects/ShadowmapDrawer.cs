...
namespace Veldrid.NeoDemo.Objects
{
    public class ShadowmapDrawer : Renderable
    {
        ...
        public override void UpdatePerFrameResources(GraphicsDevice gd, CommandBuffer cb, SceneContext sc)
        {
            if (_si.HasValue)
            {
                cb.UpdateBuffer(_sizeInfoBuffer, 0, _si.Value);
                _si = null;
            }

            if (_ortho.HasValue)
            {
                cb.UpdateBuffer(_orthographicBuffer, 0, _ortho.Value);
                _ortho = null;
            }
        }
        ...
        public override void Render(GraphicsDevice gd, CommandBuffer cb, SceneContext sc, RenderPasses renderPass)
        {
            ...
            cb.DrawIndexed((uint)s_quadIndices.Length, 1, 0, 0, 0);
        }
