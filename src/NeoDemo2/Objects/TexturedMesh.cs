...
namespace Veldrid.NeoDemo.Objects
{
    public class TexturedMesh : CullRenderable
    {
        ...
        public unsafe override void CreateDeviceObjects(GraphicsDevice gd, SceneContext sc)
        {
            ...
            _vb = _meshData.CreateVertexBuffer(disposeFactory, null);
            _vb.Name = _name + "_VB";
            _ib = _meshData.CreateIndexBuffer(disposeFactory, null, out _indexCount);
            _ib.Name = _name + "_IB";
            ...
            {
                _materialProps.CreateDeviceObjects(gd);
            }
        ...
        public override void Render(GraphicsDevice gd, CommandBuffer cb, SceneContext sc, RenderPasses renderPass)
        {
            if (_materialPropsOwned)
            {
                _materialProps.FlushChanges(cb);
            }
        ...
        private void RenderShadowMap(CommandBuffer cb, SceneContext sc, int shadowMapIndex)
        {
            cb.BindVertexBuffer(0, _vb);
            cb.BindIndexBuffer(_ib, IndexFormat.UInt16);
            cb.BindPipeline(_shadowMapPipeline);
            cb.BindGraphicsResourceSet(0, _shadowMapResourceSets[shadowMapIndex * 2]);
            Span<uint> offsets = stackalloc uint[1];
            offsets[0] = _uniformOffset;
            cb.BindGraphicsResourceSet(1, _shadowMapResourceSets[shadowMapIndex * 2 + 1], offsets);
            cb.DrawIndexed((uint)_indexCount, 1, 0, 0, 0);
        }

        private void RenderStandard(CommandBuffer cb, SceneContext sc, bool reflectionPass)
        {
            cb.BindVertexBuffer(0, _vb);
            cb.BindIndexBuffer(_ib, IndexFormat.UInt16);
            cb.BindPipeline(reflectionPass ? _pipelineFrontCull : _pipeline);
            cb.BindGraphicsResourceSet(0, _mainProjViewRS);
            cb.BindGraphicsResourceSet(1, _mainSharedRS);
            Span<uint> offsets = stackalloc uint[1];
            offsets[0] = _uniformOffset;
            cb.BindGraphicsResourceSet(2, _mainPerObjectRS, offsets);
            cb.BindGraphicsResourceSet(3, reflectionPass ? _reflectionRS : _noReflectionRS);
            cb.DrawIndexed((uint)_indexCount, 1, 0, 0, 0);
        }
    }
