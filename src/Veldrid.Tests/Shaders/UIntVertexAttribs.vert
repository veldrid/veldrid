...
{
    Veldrid_Tests_Shaders_UIntVertexAttribs_Info field_InfoBuffer;
};
...
Veldrid_Tests_Shaders_UIntVertexAttribs_FragmentInput VS( Veldrid_Tests_Shaders_UIntVertexAttribs_Vertex input_)
{
    Veldrid_Tests_Shaders_UIntVertexAttribs_FragmentInput output_;
    output_.Position = field_Ortho * vec4(input_.Position, 0, 1);
    output_.Color = vec4(input_.Color_UInt.x, input_.Color_UInt.y, input_.Color_UInt.z, 1) / field_InfoBuffer.ColorNormalizationFactor;
    output_.Color.w = 1;
    return output_;
}
