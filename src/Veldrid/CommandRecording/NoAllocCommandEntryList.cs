...
{
    internal unsafe class NoAllocCommandEntryList : IDisposable
    {
        ...
        {
            NoAllocMemoryBarrierEntry entry = new NoAllocMemoryBarrierEntry(
                Track(texture),
                baseMipLevel, levelCount,
                baseArrayLayer, layerCount,
                sourceStage,
                destinationStage);
            AddEntry(MemoryBarrierEntryID, ref entry);
        }

        private const byte BeginRenderPassEntryID = 27;
        private static readonly uint BeginRenderPassEntrySize = Util.USizeOf<NoAllocBeginRenderPassEntry>();

        private const byte EndRenderPassEntryID = 28;
        private static readonly uint EndRenderPassEntrySize = Util.USizeOf<NoAllocEndRenderPassEntry>();

        private const byte MemoryBarrierEntryID = 29;
        private static readonly uint MemoryBarrierEntrySize = Util.USizeOf<NoAllocMemoryBarrierEntry>();

        public NoAllocCommandEntryList()
        {
            _memoryPool = new StagingMemoryPool();
            ...
            for (uint i = 0; i < _totalEntries; i++)
            {
                ...
                switch (id)
                {
                    ...
                    case BeginRenderPassEntryID:
                        ...
                        break;
                    case EndRenderPassEntryID:
                        NoAllocEndRenderPassEntry erpe = Unsafe.ReadUnaligned<NoAllocEndRenderPassEntry>(entryBasePtr);
                        executor.EndRenderPass();
                        currentOffset += EndRenderPassEntrySize;
                        break;
                    case MemoryBarrierEntryID:
                        NoAllocMemoryBarrierEntry mbe = Unsafe.ReadUnaligned<NoAllocMemoryBarrierEntry>(entryBasePtr);
                        executor.MemoryBarrier(
                            mbe.Texture.Get(_resourceList),
                            mbe.BaseMipLevel, mbe.LevelCount,
                            mbe.BaseArrayLayer, mbe.LayerCount,
                            mbe.SourceStage,
                            mbe.DestinationStage);
                        currentOffset += MemoryBarrierEntrySize;
                        break;
                    default:
        ...
        {
            NoAllocEndRenderPassEntry entry = new NoAllocEndRenderPassEntry();
            AddEntry(EndRenderPassEntryID, ref entry);
        }
        ...
        public void SetGraphicsResourceSet(uint slot, ResourceSet rs, uint dynamicOffsetCount, ref uint dynamicOffsets)
        {
            ...
            {
                SetResourceSet(slot, rs, new Span<uint>(pDynamicOffsets, (int)dynamicOffsetCount), isGraphics: true);
            }
        }
        ...
        private void SetResourceSet(uint slot, ResourceSet rs, Span<uint> dynamicOffsets, bool isGraphics)
        {
            NoAllocSetResourceSetEntry entry;

            if ((uint)dynamicOffsets.Length > NoAllocSetResourceSetEntry.MaxInlineDynamicOffsets)
            {
                StagingBlock block = _memoryPool.GetStagingBlock((uint)dynamicOffsets.Length * sizeof(uint));
                _stagingBlocks.Add(block);
                for (uint i = 0; i < (uint)dynamicOffsets.Length; i++)
                {
                    *((uint*)block.Data + i) = dynamicOffsets[(int)i];
                }
            ...
            {
                entry = new NoAllocSetResourceSetEntry(slot, Track(rs), isGraphics, dynamicOffsets);
            }
        ...
        public void SetComputeResourceSet(uint slot, ResourceSet rs, uint dynamicOffsetCount, ref uint dynamicOffsets)
        {
            fixed (uint* pDynamicOffsets = &dynamicOffsets)
            {
                SetResourceSet(slot, rs, new Span<uint>(pDynamicOffsets, (int)dynamicOffsetCount), isGraphics: false);
            }
        }
