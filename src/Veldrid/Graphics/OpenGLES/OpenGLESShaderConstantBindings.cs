...
namespace Veldrid.Graphics.OpenGLES
{
    public class OpenGLESShaderConstantBindings : ShaderConstantBindings
    {
                ...
                if (blockIndex != -1)
                {
                    ...
                    _globalUniformBindings[i] = new GlobalBindingPair(
                        new UniformBlockBinding(
                            ...
                            blockIndex,
                            bindingIndex,
                            new OpenGLESConstantBuffer(dataProvider),
        ...
        private class UniformBlockBinding : UniformBinding
        {
            ...
            public OpenGLESConstantBuffer ConstantBuffer { get; }

            public UniformBlockBinding(
                int programID,
                int blockIndex,
                int bindingIndex,
                OpenGLESConstantBuffer constantBuffer,
                int dataSizeInBytes)
                : base(programID)
            {
        ...
        private class UniformLocationBinding : UniformBinding
        {
            ...
            {
                StorageAdapter = new OpenGLESUniformStorageAdapter(ProgramID, uniformLocation);
            }
